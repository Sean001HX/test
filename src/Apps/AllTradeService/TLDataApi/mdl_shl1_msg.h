...
struct Indexes {
    ...
    MDLAnsiString IndexID;           // 指数ID,股票代码:eg"000001"
	MDLUTF8String IndexName;
...
struct Equity {
	...
	MDLAnsiString SecurityID;           // 证券ID,股票代码:eg"000001"
	MDLUTF8String SecurityName;         // 证券名  eg: 平安银行
    MDLAnsiString TradingPhaseCode;
	MDLTime UpdateTime;
	uint64_t Volume;                    // 成交量
	MDLDoubleT<2> Turnover;             // 成交额,2位小数
	MDLDoubleT<3> PreCloPrice;          // 昨结算/昨收盘,昨收盘*1.1=涨停价,余数进位;昨收盘*0.9=跌停价,余数进位
	MDLDoubleT<3> OpenPrice;            // 开盘价
	MDLDoubleT<3> HighPrice;            // 当日最高价
	MDLDoubleT<3> LowPrice;             // 当日最低价
	MDLDoubleT<3> LastPrice;            // 最新价
	MDLDoubleT<3> ClosePrice;           // 今收盘价
	struct BidPriceLevelItem {
		uint64_t Volume;
		MDLDoubleT<3> Price;
	};
	MDLListT<BidPriceLevelItem> BidPriceLevel;  // 五档买
	struct AskPriceLevelItem {
		uint64_t Volume;
		MDLDoubleT<3> Price;
	};
    MDLListT<AskPriceLevelItem> AskPriceLevel;  // 五档卖
};
