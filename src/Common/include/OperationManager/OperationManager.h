#ifndef OPERATION_MANAGER_H_
#define OPERATION_MANAGER_H_
...
#ifdef OPERATIONMANAGER_EXPORTS
#define OPERATIONMANAGER_API __declspec(dllexport)
#else
#define OPERATIONMANAGER_API __declspec(dllimport)
#endif
...
namespace OperationManager
{
	...
	class OPERATIONMANAGER_API COperationManager :public CommonTools::instance_shared_from_this<COperationManager>
	{
	public:
		COperationManager();
		~COperationManager();
...
#pragma region 用户与交易所管理相关操作
        ...
        // 获取用户资金信息
        bool getUserMoneyInfo(const UserIDType user_id, AllTrade::NPMgrCenter::SMoneyInfoPtr& money_info);
        // 获取用户风控信息
        bool getUserRiskInfo(const UserIDType user_id, AllTrade::NPMgrCenter::SRiskInfoPtr& risk_info);

        // 获取所有交易所信息
        bool getAllExchange(std::vector<AllTrade::NPMgrCenter::SExchangePtr>& exchanges);
        // 获取所有交易所信息
        bool getStockAllContract(std::vector<AllTrade::NPMgrCenter::SStockContractPtr>& contract_vec, const ExchangeIDType exchange_id);
        ...
        bool updateUserMoneyInfo(const UserIDType user_id, const AllTrade::NPMgrCenter::SMoneyInfoPtr& money_info);
        // 创建/更新用户风控信息
        bool updateUserRiskInfo(const UserIDType user_id, const AllTrade::NPMgrCenter::SRiskInfoPtr& risk_info);

        // 创建/更新子账户入金审核
        bool qryApplyMoney(std::vector<AllTrade::NPMgrCenter::SApplyMoneyPtr>& apply_infos, const AllTrade::UserIDType user_id, const DateType start_day, const DateType end_day, AllTrade::OIMM_Change_Type money_type);
...
#pragma endregion
        ...
        // 黑白名单相关
        void addStockBlackWhite(const ContractIDType code);
